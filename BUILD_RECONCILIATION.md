...
- **Treat the LLM as untrusted input — parse strictly at the boundary.** Today `parseProposal` (`judge-cli.ts`) is *tolerant* (strips fences, greps the JSON). That's fine for extraction, but the result must then pass a **strict zod schema** before it can mutate a board. A malformed or hallucinated proposal should fail loud, not commit silently (canon: "parse/validate data where it enters; trust it inside"). Define the judge's output contract as a zod schema in `core` and validate every proposal against it.
