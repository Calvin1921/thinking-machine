...
the five heuristics (below) at EVERY level — then commit the entire subtree in ONE `tmind grow`
...
the WHOLE tree at once, not per level. Then `tmind grow`.

Use **shallow / co-build mode** (one level via `tmind decompose`, confirm, iterate) only when the
...
rationale, get one yes, then **`tmind grow`** the whole thing in a single call (use `tmind decompose`
...
Turn on with `tmind guide on`. In Guide mode, follow this loop. The machine does the OBJECTIVE
...
0. **LOCATE** (only when the user has no question — fog). Propose 3–6 candidate signals/questions
   ...
   center — never hand back a menu. Seed it with `tmind init "<center>"` (or `tmind new`).
1. **WIDEN** (default first move). Propose a SHALLOW breadth map — the major parts, ~5–9 peers,
   one level, no recursion. Before committing, run `tmind collisions --labels "A,B,C"` and resolve
   each hit (see Duplicate resolution below). Commit with `tmind decompose <nodeId> --json '{...}'`,
   then `tmind provenance <id> drafted` on new nodes.
...
3. **DEEPEN** (vertical). On the chosen node, propose options with a "pick this if X" rationale,
   then `tmind grow <nodeId> --json '{...}'` along that spine toward atoms. Collapse siblings. Mark
   new nodes `drafted` (Phase 1 does no verification).
4. **PRIORITISE**. Use `tmind status <id> <todo|running|...>` to mark what matters; this is the
   user's call.
...
When `tmind collisions` reports a proposed label already on the board, ask the user which it is:
...
|---|---|---|
| Enter Guide mode | `tmind guide on` | Sets `guideMode: true` on the board |
| Exit Guide mode | `tmind guide off` | Clears flag; reverts to deep-dive default |

Guide mode is a board-level flag — it persists across sessions until turned off. The CLI table
above (`tmind show`) does not surface it directly; use `tmind show --json` and check `guideMode`.
...
1. CACHE CHECK (cross-board, context-aware): `tmind cache-entry "<topic>"`. On a HIT, read its
   ...
   `tmind grow`). If it DIFFERS (e.g. cached from "static blog", now on a "video platform"),
...
2. DRAFT + RENDER. `tmind grow <nodeId> --json '{...}'`, then `tmind provenance <id> drafted` on
...
4. VERIFY in the background, upgrading in place:
   - FACTUAL → WebSearch the claim, judge it, then
     `tmind verify <id> --provenance verified --kind factual --sources <url1,url2> --volatility <static|weeks|volatile>`.
   ...
   - SUBJECTIVE → present 2–3 competing credible takes ("pick this if X"), then
     `tmind verify <id> --provenance informed-opinion --kind subjective`. Never fake `verified`.
   - If sources are thin/uncertain → leave it `drafted` (honest), don't upgrade.
5. CACHE WITH CONTEXT: `tmind cache-put "<topic>" --json '{...}' --context "<this board's context>"`
...
6. RATIONALE WRITEBACK. For each option node you verified in step 4 (and any grafted from cache),
   write its "pick this if X" rationale: `tmind rationale <id> "pick this if …"`. This renders in the node drawer alongside provenance
   and sources, so the user sees the decision logic without re-reading the thread.
7. STALENESS. Run `tmind refresh-stale` when revisiting a board; `stale` nodes should be
